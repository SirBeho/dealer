...
function handleFavoriteClick(event, vehicleId, userId) {
  event.preventDefault();

  var xhr = new XMLHttpRequest();
  var url = `../controller/favoritos.php?id_u=${userId}&id_v=${vehicleId}`;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (xhr.responseText == "null") {
        console.log("si");
        event.target.src = "../svg/favorito.svg";
      } else {
        console.log("no");
        event.target.src = "../svg/nfavorito.svg";
